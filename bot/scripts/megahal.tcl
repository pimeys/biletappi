...
#setmegabotnick $nick
setmegabotnick AAAAAAAAAAAAAAAAAAAAAAA$nick
...
proc pub_lobotomy {nick uhost hand chan arg} {
 ...
 reloadbrain
 savebrain
 putmsg $chan "Lobotomy completed! Creating a new brain..." 
}
...
proc pub_braininfo {nick uhost hand chan arg} {
  ...
  putmsg $chan "My current vocabulary consists of [lindex $for 0] words, my brain size is [expr [lindex $for 1]+[lindex $back 1]] nodes, and learning mode is $learnmode" 
...
proc pub_learningmode {nick uhost hand chan arg} {
  ...
  if {$arg1 == "" || ($arg1 != "on" && $arg1 != "off")} {
   putmsg $chan "Usage: .learningmode on/off"
   return
  }
  set learnmode $arg1
  learningmode $learnmode
  putmsg $chan "Learning mode is now set to $arg1" 
}
...
proc pub_talkfrequency {nick uhost hand chan arg} {
  ...
  set arg1 [lindex $arg 0]
  if {$arg1 == ""} {
   putmsg $chan "Talking frequency is set to $talkfreq lines" 
   return
  }
  set talkfreq $arg1
  putmsg $chan "Talking frequency is now set to $arg1 lines" 
}
...
proc pub_restorebrain {nick uhost hand chan arg} {
  if {[file exists megahal.old]} {
    ...
    reloadbrain
    putmsg $chan "Old brain restored!"
  } else {
    putmsg $chan "Old brain not found!" 
  }
}
